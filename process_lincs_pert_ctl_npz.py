...
def load_and_eda(DATA_ROOT, true_pert_type, outdir, log_handle):
    ...
    inst_info_oe = inst_info[np.logical_and(inst_info['pert_type'] == true_pert_type, inst_info['failure_mode'].isna()) ].reset_index(inplace=False, drop=True)
    
    # reads in oe matrix save as pd df
    if true_pert_type == "trt_xpr":
        pert_file_name = "level3_beta_trt_xpr_n420583x12328.gctx"
    else:
        pert_file_name = "level3_beta_trt_oe_n131668x12328.gctx"
         
    f = h5py.File(os.path.join(DATA_ROOT, pert_file_name), 'r')
...
def filter_by_cell_line(inst_info_oe, matrix_oe, inst_info_ctl, matrix_ctl, gene_info, log_handle, true_pert_type):
...
def combine_lognorm_save(inst_info_oe, matrix_oe, inst_info_ctl, matrix_ctl, log_handle, outdir, keep_cell_lines, true_pert_type):
    ...
    for cell_line in keep_cell_lines:
        for pert_type in [true_pert_type, 'ctl_vector']:
...
def main():
    ...
    parser.add_argument('--true_pert_type', type=str, required=True,
                        help='Name of the perturbation file')
    args = parser.parse_args()
    
    # get the variables
    DATA_ROOT = args.data_dir
    outdir = args.out_dir
    true_pert_type = args.true_pert_type
    ...
    # begin processing
    inst_info_oe, matrix_oe, inst_info_ctl, matrix_ctl, gene_info, log_handle = load_and_eda(DATA_ROOT, true_pert_type, outdir, log_handle)
    inst_info_oe, matrix_oe, inst_info_ctl, matrix_ctl, gene_info, log_handle, keep_cell_lines = filter_by_cell_line(inst_info_oe, matrix_oe, inst_info_ctl, matrix_ctl, gene_info, log_handle, true_pert_type)
    combine_lognorm_save(inst_info_oe, matrix_oe, inst_info_ctl, matrix_ctl, log_handle, outdir, keep_cell_lines, true_pert_type)
