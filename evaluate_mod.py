...
# Load modified model
bothmod_pert_model = torch.load("/home/b-evelyntong/hl/training_history/train_0425_bothmod_2_30_A/oe/A375/_fold_1_perturbation_discovery.pt")
...
model_modified = PDGrapher(
    edge_index,
    model_kwargs={"n_layers_nn": 2, "n_layers_gnn": 2, "positional_features_dim": 64, "embedding_layer_dim": 8, "dim_gnn": 8, "num_vars": 7632},
    precomputed_embeddings_path=embedding_path
    )

model_modified.perturbation_discovery.load_state_dict(bothmod_pert_model['model_state_dict'], strict=True)
model_modified.perturbation_discovery.edge_index = model_modified.perturbation_discovery.edge_index.to(device)
model_modified.perturbation_discovery = model_modified.perturbation_discovery.to(device)
model_modified.perturbation_discovery.eval()
...
for idx, data in enumerate(tqdm(test_loader_backward)):
    ...
    with torch.no_grad():
        out = model_modified.perturbation_discovery(
            input_tensor, batch, mutilate_mutations=mutations, threshold_input=thresholds
        )

    modified_preds.append(out.detach().cpu())

torch.save(modified_preds, "/home/b-evelyntong/hl/modified_preds.pt")
print("Saved modified predictions to modified_preds.pt")
